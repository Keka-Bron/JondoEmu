...
namespace Jondo.Unity.Tests.Diagnostics
{
    public class ActivityJournalTests
    {
        ...
        public void Invalid_events_and_unserializable_details_cannot_break_gameplay()
        {
            ...
            // An event with no name is not an event, so nothing is written for the first call. But
            // the second one HAS to leave a line: this is an audit log, and losing the whole event
            // because its detail would not serialise takes the name, the account and the character
            // down with it. What failed is said in the line instead of swallowed.
            string line = Assert.Single(lines);
            using var json = JsonDocument.Parse(line);
            Assert.Equal("broken.detail", json.RootElement.GetProperty("event").GetString());
            Assert.True(json.RootElement.GetProperty("details")
                            .TryGetProperty("journalError", out _));
        }

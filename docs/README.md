...
## 📂 Cómo está repartida la carpeta

La raíz no tiene ficheros sueltos a propósito: quien se baje el emulador ve el `.exe` y poco más,
sin tener que adivinar qué abrir.

```
Jondo Emulator Launcher.exe   ← esto es lo que se ejecuta, y no hay nada más que ejecutar
datos/                        json y bin que el emulador lee (mapas, objetos, apariencias, zaaps…)
bases/                        world.db y auth.db, que son lo único que el emulador escribe
docs/                         esta documentación
logs/                         registros y volcados de trabajo
launcher_assets/              imágenes y música del lanzador
tools/                        las herramientas de Python que generan lo de datos/
dofus3_data/                  volcado en crudo del cliente, del que salen los datos
Jondo.Unity.*/ , JondoFix/    el código fuente
```

El `.exe` va en un solo fichero: las doce dll de antes (Protobuf, Thrift, SQLite y compañía) están
dentro. Sigue haciendo falta el runtime de **.NET 10** instalado.

Los ficheros se buscan primero en `datos/`, luego en `bases/` y por último en la raíz, así que una
instalación a medio mover arranca igual.
...
The emulator uses two **SQLite** databases for local persistence, both in `bases/`:
* **`auth.db`**: Stores accounts, credentials, and authentication sessions.
* **`world.db`**: Distributed compressed as `datos/world.zip` (unzip after cloning). Stores character data, inventories, positions, map persistence, spells, monsters, and world states.
...
2. Extract the `world.zip` archive into `bases/` to generate `world.db`:
   ```powershell
   powershell Expand-Archive -Path datos\world.zip -DestinationPath bases -Force
   ```

...
*   **Problemas Identificados**:
    ...
    2.  **El ID en el Cliente**: Sin embargo, durante el login, el emulador envÃ­a la lista de personajes `ksq` utilizando bytes pregrabados estÃ¡ticos de la captura oficial, donde el personaje (originalmente PLANTILLA en el PCAP, ahora CADERNIS en la DB) tiene el ID oficial de Ankama: `13825558L`.
...
*   **Estado de CompilaciÃ³n y Despliegue**:
    - **JondoFix Mod**: CompilaciÃ³n exitosa en modo Release (0 errores), DLL desplegada correctamente en [JondoFix.dll](file:///C:/Jondo/DofusClient/Mods/JondoFix.dll).
    - **Emulador Launcher**: CompilaciÃ³n de la soluciÃ³n `Jondo.Unity.sln` exitosa en modo Release (0 errores), y archivos `.bin` sincronizados en los directorios Debug/Release de net10.0.
*   **Resultados Esperados**: Al conectarse, Harmony aplicarÃ¡ el parche de chat sin advertencias. Al cargar el mapa, el emulador reescribirÃ¡ con Ã©xito el nombre del personaje en el tercer nivel de anidaciÃ³n en `jpv`, logrando que al pasar el mouse por encima del personaje se renderice el nombre `"CADERNIS"` en lugar de `"PLANTILLA"`, completando al 100% la carga del personaje.

*   **Resultados Obtenidos**: **FRACASO**. Aunque parcheamos el nombre del personaje en el hover al parsear `jpv_packet.bin` en la carga del mapa (`MapLoadHandler`), el cliente seguÃ­a mostrando `"PLANTILLA"` al pasar el mouse por encima. AdemÃ¡s, MelonLoader registrÃ³ una excepciÃ³n fatal `NullReferenceException` en el mÃ©todo `eud.bcoh` de cartografÃ­a, provocando el cierre de los hilos de red y haciendo fallar los handshakes del ChatServer.
    Se descubriÃ³ que:
    1. El nombre `"PLANTILLA"` se envÃ­a al cliente en un paquete `jpv` que forma parte del conjunto inicial de 17 paquetes de entrada al mundo (`world_entering_packets.bin`), el cual era enviado directamente por `GameNodeProxy.cs` sin aplicar ninguna clase de parche.
...
*   **Resultados Obtenidos**: **FRACASO**.
    ...
    2. El hover del nombre del avatar local seguÃ­a mostrando `"PLANTILLA"`. Esto ocurrÃ­a porque:
...
## Iteración #209 - Migración a Versión 3.6.10.10, Redirección kqz/kra, Protocolo kqu/kvi y Creación del Personaje [#DRAGON-LORD#] (2026-08-11)

### 1. Diagnóstico de Defectos en Versión 3.6.10.10
- **Bloqueo tras Selección de Servidor**: El cliente 3.6.10.10 utiliza `kqz` para solicitar el servidor y espera `kra` como confirmación de redirección en lugar de la envoltura binaria previa.
- **Protocolo de Selección de Personajes 3.6.10.10**: El Game Node debe iniciar con el saludo `hoy`. La petición de personajes del cliente viaja en `kqu` (en lugar de `kpa`/`knx`). La respuesta requiere la secuencia `mgq` ➔ `mgt` ➔ `hpd` ➔ `krs` ➔ `mgz` ➔ `kqp` ➔ `kvi` ➔ `jtg`.
- **Eliminación de Datos Hardcodeados**: Requisito explícito de contar con múltiples personajes reales en SQLite (`world.db`) asociados a la misma cuenta para comprobar el renderizado correcto de la lista sin depender de capturas estáticas.

### 2. Modificaciones Realizadas en C# y Base de Datos
1. **OpcodeRegistry.cs**: Agregados opcodes nativos de 3.6.10.10 (`kqz`, `kra`, `hoy`, `kqu`, `kvi`, `kvw`, `kva`).
2. **GameServerProxy.cs**: Implementado el receptor de `kqz`/`krt` devolviendo `kra` en el puerto `5555`.
3. **GameNodeProxy.cs**: Emitido saludo `hoy` al conectar la sesión TCP del cliente y enrutados los opcodes `kqu` y `kvw`.
4. **CharacterSelectionHandler.cs**: Implementado el método `BuildKviPacket(dbChars)` para empaquetar dinámicamente la lista de personajes en la estructura Protobuf de `kvi`.
5. **DatabaseManager.cs & SQLite (`world.db`)**:
   - Agregada la propiedad `AccountId` a `DbCharacter`.
   - Creado el personaje `[#DRAGON-LORD#]` (Id `13825559`, AccountId `188940901`, Breed `9` Cra, Nivel `50`, Kamas `1351260`) replicando todos los equipamientos y estadísticas de `[#KEKA-BRON#]` en las tablas `Characters` y `CharacterItems`.

### 3. Verificación de Compilación Dual y Publicación
- **Debug**: `dotnet build "C:\Jondo 3.6.10.10\Jondo Unity Emulator\Jondo.Unity.sln" -c Debug` -> **0 Errores** ✅
- **Release**: `dotnet build "C:\Jondo 3.6.10.10\Jondo Unity Emulator\Jondo.Unity.sln" -c Release` -> **0 Errores** ✅
- **Despliegue**: Binarios actualizados y copiados en el directorio raíz `C:\Jondo 3.6.10.10\Jondo Unity Emulator\`.

- **Corrección de Conteo de Personajes en Servidores (`GameServerProxy.cs`)**: El mensaje de lista de servidores (`AuthenticationTicketAccepted`) rellenaba únicamente `sInfo.Characters[0]` con 1 solo personaje. Se actualizó `GetModifiedAuthAcceptedMessage()` para iterar sobre todos los personajes encontrados en la DB (`dbChars`), vaciar `sInfo.Characters` y añadir dinámicamente cada `CharacterInfo` (`[#KEKA-BRON#]` y `[#DRAGON-LORD#]`). Ahora la tarjeta del servidor muestra correctamente `PERSONAJE (2)` en la pantalla de selección de servidores.


---

## Iteración #210 - Launcher Nativo Gráfico Estilo Dofus 2 Old-School, Protecciones Anti-SQLi/Anti-DDoS y Aislamiento de Cuentas keka/dragonlord (2026-08-13)

### 1. Requerimientos Implementados
- **Launcher Nativo Gráfico Dofus 2**: Diseño de interfaz propio con pergaminos oscuros medievales, bordes tallados y animaciones flotantes relucientes de los **Huevos Dofus Primordiales**.
- **Seguridad Anti-SQL Injection**: Consultas parametrizadas en SQLite (`auth.db` y `world.db`) y sanitización estricta por Expresiones Regulares (`Regex`).
- **Defensa Anti-DDoS y Rate Limiting**: Limitador de peticiones por IP y bloqueo temporal (cooldown de 60 segundos) tras 5 intentos fallidos de autenticación.
- **Configuración Multijugador (2 Clientes)**: Separación de la cuenta `keka` / `test` (`188940901` -> `[#KEKA-BRON#]`) y la cuenta `dragonlord` / `test` (`188940902` -> `[#DRAGON-LORD#]`) para pruebas concurrentes.

### 2. Verificación de Compilación y Publicación
- **Debug**: `dotnet build "C:\Jondo 3.6.10.10\Jondo Unity Emulator\Jondo.Unity.sln" -c Debug` -> **0 Errores** ✅
- **Release**: `dotnet build "C:\Jondo 3.6.10.10\Jondo Unity Emulator\Jondo.Unity.sln" -c Release` -> **0 Errores** ✅
- **Despliegue**: Binarios y assets gráficos copiados en `C:\Jondo 3.6.10.10\Jondo Unity Emulator\`.
